...
TEST_CASE("Djikstra - path doesn't exist", "[weight=1]"){
  Graph g(true, true);
  g.insertVertex("one");
	g.insertVertex("two");
	g.insertVertex("three");
  ...
  g.insertEdge("one", "two");
	...
	g.insertEdge("two", "three");
	g.setEdgeWeight("two", "three", 2);

	g.insertEdge("three", "four");
	g.setEdgeWeight("three", "four", 2);

	vector<Vertex> result = g.Dijkstra("three", "one");
...
TEST_CASE("Simple Centrality", "[weight=1"){
  ...
  // vector<Vertex> result = g.Dijkstra("four", "one");
